...
public class PlayerWeaponsManager : MonoBehaviour
{
    ...
    public bool pointingAtEnemy { get; private set; }
    ...
    List<WeaponController> m_WeaponSlots = new List<WeaponController>(9); // 9 available weapon slots
    ...
    private void Update()
    {
        ...
        if (activeWeapon && m_WeaponSwitchState == WeaponSwitchState.Up)
        {
            ...
            // handle shooting
            bool hasFired = activeWeapon.HandleShootType(
        ...
        if (!aiming &&
            (activeWeapon == null || !activeWeapon.charging) &&
            (m_WeaponSwitchState == WeaponSwitchState.Up || m_WeaponSwitchState == WeaponSwitchState.Down))
        {
            int switchWeaponInput = 0;
            if (Input.GetAxis("Mouse ScrollWheel") > 0 || Input.GetAxis("Change Weapon") > 0) switchWeaponInput = 1;
            else if (Input.GetAxis("Mouse ScrollWheel") < 0 || Input.GetAxis("Change Weapon") < 0) switchWeaponInput = -1;
    ...
    private int GetSelectWeaponInput()
    {
    ...
    public void SwitchWeapon(bool ascendingOrder)
    {
        ...
        int closestSlotDistance = m_WeaponSlots.Count;
    ...
    void UpdateWeaponBob()
    {
        if (Time.deltaTime > 0f)
        {
            ...
            float bobAmount = aiming ? aimingBobAmount : defaultBobAmount;
    ...
    int GetDistanceBetweenWeaponSlots(int fromSlotIndex, int toSlotIndex, bool ascendingOrder)
    {
        ...
        if (ascendingOrder) distanceBetweenSlots = toSlotIndex - fromSlotIndex;
        else distanceBetweenSlots = -1 * (toSlotIndex - fromSlotIndex);
        
        if (distanceBetweenSlots < 0) distanceBetweenSlots = m_WeaponSlots.Count + distanceBetweenSlots;
